...
class SdVolume {
 public:
  ...
  if(dev->_type == TYPE_SPI_W25Q) return init(dev, 0);
  else return init(dev, 1) ? true : init(dev, 0);
  }
...
//
  ...
  uint8_t isEOC(uint32_t cluster) const {
    if(sdCard_->_type == TYPE_SPI_W25Q) return cluster >= 0xFF8;
    else return cluster >= (fatType_ == 16 ? FAT16EOC_MIN : FAT32EOC_MIN);
  }
