...
//------------------------------------------------------------------------------
#define TYPE_SPI_SD      0//用的SPI和SD卡通信
#define TYPE_SPI_W25Q    1//用的SPI和W25Q flash通信
#define TYPE_SDIO_SD     2//用的SDIO和SD卡或者flash通信
...
class Sd2Card {
 public:
  ...
  Sd2Card() : _type(TYPE_SPI_SD), errorCode_(0), inBlock_(0), partialBlockRead_(0), type_(0) {}
   ...
   */
  uint8_t init(uint8_t storageType = TYPE_SPI_SD) {
    return init(SPI_FULL_SPEED, SD_CHIP_SELECT_PIN,storageType,0);
  }
  /**
   ...
   */
  uint8_t init(uint8_t sckRateID, uint8_t storageType) {
    return init(sckRateID, SD_CHIP_SELECT_PIN, storageType, 0);
  }
  uint8_t init(uint8_t sckRateID, uint8_t chipSelectPin, uint8_t storageType, uint32_t freq = 0);
...
private:
  ...
  uint8_t chipSelectPin_;//SPI的片选引脚
  uint8_t errorCode_;//错误代码
  uint8_t inBlock_;
  uint16_t offset_;
